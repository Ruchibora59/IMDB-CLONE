...
def ForbiddenUsers(value):
    forbidden_users =['admin', 'css' ,'js', 'authenticate','login','logout', 'root', 'administrator', 'email' ,'join', 'sql', 'insert', 'db', 'static', 'python', 'delete', 'TABLE', 'insert']
...
def InvalidUser(value):
    if '@' in value or '+' in value or '-' in value:
        raise ValidationError('This is an invalid user, do not use chars: @,-,+')

def UniqueEmail(value):
    if User.objects.filter(email_iexact=value).exists():
        raise ValidationError('User with this email already ecxists. ')
...
class ChangePasswordForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not user.check_password(old_password):
            self.errors['old_password'] = self.error_class(['Old Passwords do not match. try Again'])
